...
def rand_search(classifier_tuple, param_grid, cv, prepp_, prep_val_, scoring=None):
    ...
    # Problem with MLPC that it says it is not trained using
    # feature names.
...
def box_plot(perf_dict, perf_meassure):
    ...
    for key, value in perf_dict.items():
            ...
            labels.append(key)
    #ax.boxplot(total, labels=perf_dict.keys())
    ...
    #fig.savefig('Accuracies_boxplot')
...
if __name__ == '__main__':
    ...
    n_pang= args.npang
    n_snps=args.nsnps
    clustf="data/kma_clustering/gov_cluster_assignments_0.98.txt"
    ...
    clust=clust.rename(index = lambda x: x.split('.fa')[0]).sort_index()
    ...
    if "virgenes" in os.path.basename(indf):
        ...
        rand_estimators={
            ...
            'nn': [MLPClassifier(max_iter=10000, early_stopping=True, random_state=1333), {'nn__hidden_layer_sizes': randint(30,110),
    ...
    if "pangenes" in os.path.basename(indf):
        ...
        grid_estimators={
            'svc_lin': [SVC(probability=True, max_iter=50000, random_state=(33)),{'svc_lin__kernel': ['linear'], 
                                               'svc_lin__C': [0.0005,0.001,0.05,0.1,0.2,0.5,1]}]
            }
        ...
        rand_estimators={
            ...
            'rf': [RandomForestClassifier(random_state=3), {'rf__n_estimators': randint(120,180),
                                                            'rf__criterion': ['gini', 'entropy'],
                                                            'rf__max_features': ['sqrt', 'log2'],
                                                            'rf__max_depth': randint(2,5),
                                                            'rf__min_samples_split': randint(25,35),
                                                            'rf__min_samples_leaf': randint(15,25),
                                                            'rf__bootstrap': [True, False]}],
            
            'nn': [MLPClassifier(max_iter=10000, early_stopping=True, random_state=1333), {'nn__hidden_layer_sizes': randint(5,15),
                                                                                           'nn__activation': ['identity', 'logistic', 'tanh', 'relu'],
                                                                                           'nn__solver': ['lbfgs', 'sgd', 'adam'],
                                                                                           'nn__alpha': loguniform(1e-16, 1e-12),
                                                                                           'nn__tol': loguniform(1e-7,1e-3),
                                                                                           'nn__n_iter_no_change': randint(400,800)}],
            
            'logit': [GradientBoostingClassifier(random_state=(33)), {'logit__n_estimators': randint(70,100),
                                                     'logit__criterion': ['friedman_mse', 'squared_error'],
                                                     'logit__learning_rate': [0.01],
                                                     'logit__loss': ['log_loss'],
                                                     'logit__max_features': ['sqrt', 'log2'],
                                                     'logit__max_depth': randint(1,7),
                                                     'logit__min_samples_split': randint(25,35),
                                                     'logit__min_samples_leaf': randint(15,25)}]
            }
    ...
    if "snps" in os.path.basename(indf):
        ...
        # pickle the new input
        # somehow it is way faster if you do it like this 
        # instead of doing read_pickle alone
        print("loading...")
        inp=pd.read_pickle(indf)
        print("done loading")
        ...
        rand_estimators={
            ...
            'nn': [MLPClassifier(max_iter=50000, early_stopping=True, random_state=1333), {'nn__hidden_layer_sizes': randint(10,13),
                                                                                           'nn__activation': ['identity', 'logistic', 'tanh', 'relu'],
                                                                                           'nn__solver': ['lbfgs', 'sgd', 'adam'],
    ...
    # bin the clinical frequencies into 3 groups
    ...
    # make a heat map of input
    if mapper=="blast" or mapper=="kma" :
    ...
    # do the repeated model training
    for d,(train_idxs, test_idxs) in enumerate(cv_outer_filt):
        ...
        voting_class.fit(X_train, np.ravel(y_train))
    ...
    # finally get the best of the best models and then fit them on entire dataset
